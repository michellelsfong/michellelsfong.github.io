...
function reset() {
  for (var i = 0; i < 3; i++) {
    movers[i] = new Mover(random(0.5, 3), 40+i*70, h); //location
  }
}
...
Mover.prototype.checkEdges = function() {
  if (this.position.y > (height - this.mass*end)) {
    // A little dampening when hitting the bottom
    this.velocity.y *= -0.9;
    this.position.y = (height - this.mass*end);
  }
};